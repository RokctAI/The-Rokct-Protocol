...
def send_registry_emails():
    ...
    updates = {
        "Equity": [
            f
            for f in published_dir.glob("01_Equity_*.xlsx")
            if datetime.fromtimestamp(f.stat().st_mtime) > week_ago
        ],
        "Grants": [
            f
            for f in published_dir.glob("02_Grants_*.xlsx")
            if datetime.fromtimestamp(f.stat().st_mtime) > week_ago
        ],
        "Tenders": [
            f
            for f in published_dir.glob("03_Tenders_*.xlsx")
            if datetime.fromtimestamp(f.stat().st_mtime) > week_ago
        ],
    }
    ...
    with open(recipient_file, "r") as f:
        for line in f:
            ...
            if attachments:
                send_email(
                    email, attachments, smtp_server, smtp_port, smtp_user, smtp_pass
                )
