...
# --- App-owned custom/ folder -------------------------------------------------
# lib/ is generated by compose and untracked. App-specific code lives in a
# tracked custom/ folder at the app root instead. Compose mirrors it to
# lib/custom/ (so package: imports and auto_route codegen see it) and wires it
# in through fixed hooks:
#   custom/routes.dart  -> `final List<AutoRoute> customRoutes` spread into
#                          the router's @generated-routes block
#   custom/theme.dart   -> `void applyCustomTheme()` called in main()'s
#                          @generated-brandhook block, after any SDK brand hook
#   composer.json "page_overrides": {"LoginRoute": "CustomLoginRoute"} swaps
#                          the page of the SDK/host route using LoginRoute.page
# With no custom/ folder and no page_overrides nothing changes.
CUSTOM_DIRNAME = "custom"
CUSTOM_MIRROR_MARKER = ".rokct_custom_mirror"
CUSTOM_ROUTES_IMPORT_ALIAS = "custom_routes"
CUSTOM_THEME_IMPORT_ALIAS = "custom_theme"


def custom_dir():
    return os.path.join(PROJECT_ROOT, CUSTOM_DIRNAME)


def has_custom_file(name):
    return os.path.isfile(os.path.join(custom_dir(), name))


def sync_custom_dir():
    """Mirror <app>/custom/ into lib/custom/, replacing the previous mirror.
    When custom/ is gone, a previous mirror (recognised by its marker file)
    is removed; a lib/custom/ compose did not create is never touched."""
    src = custom_dir()
    dest = os.path.join(PROJECT_ROOT, "lib", CUSTOM_DIRNAME)
    is_mirror = os.path.isfile(os.path.join(dest, CUSTOM_MIRROR_MARKER))
    if not os.path.isdir(src):
        if is_mirror:
            shutil.rmtree(dest)
            print("[*] custom/ removed - dropped its lib/custom/ mirror")
        return
    if os.path.exists(dest) and not is_mirror:
        compose_warning(
            f"lib/{CUSTOM_DIRNAME}/ exists but was not created by compose; "
            f"custom/ NOT mirrored over it"
        )
        return
    if os.path.exists(dest):
        shutil.rmtree(dest)
    shutil.copytree(src, dest)
    with open(os.path.join(dest, CUSTOM_MIRROR_MARKER), "w", encoding="utf-8") as f:
        f.write("Generated by compose from custom/. Edit custom/, not this copy.\n")
    print("[*] Mirrored custom/ into lib/custom/")


def get_page_overrides():
    """composer.json "page_overrides": {"<SDK route class>": "<custom route
    class>"}; either side may carry the ".page" suffix. Returns {} if unset."""
    composer_json_path = os.path.join(PROJECT_ROOT, "composer.json")
    if not os.path.exists(composer_json_path):
        return {}
    try:
        with open(composer_json_path, "r", encoding="utf-8-sig") as f:
            raw = json.load(f).get("page_overrides", {}) or {}
    except Exception as e:
        compose_warning(
            f"unreadable composer.json {composer_json_path} ({e}); "
            f"page_overrides NOT applied"
        )
        return {}
    if not isinstance(raw, dict):
        compose_warning("composer.json page_overrides is not an object; NOT applied")
        return {}

    def bare(v):
        v = str(v).strip()
        return v[: -len(".page")] if v.endswith(".page") else v

    return {bare(k): bare(v) for k, v in raw.items() if str(v).strip()}


def apply_page_override(page, overrides, used):
    """page is a route entry's page expression, e.g. "LoginRoute.page"."""
    if not overrides or not page:
        return page
    key = page[: -len(".page")] if page.endswith(".page") else page
    if key in overrides:
        used.add(key)
        return f"{overrides[key]}.page"
    return page
...
def update_router_table():
    ...
    for pkg_name, pkg_data in state.get("packages", {}).items():
        ...
        for r in pkg_routes:
            path = r.get("path")
            page = apply_page_override(r.get("page"), overrides, used_overrides)
    ...
    for key in sorted(set(overrides) - used_overrides):
        # Informational only: the router is regenerated after each SDK
        # installs, so an SDK installed later may still provide the route.
        print(
            f"  [i] page_overrides: no installed SDK or host route uses {key}.page yet"
        )

    # App-owned extra routes from custom/routes.dart (mirrored to lib/custom/).
    if has_custom_file("routes.dart"):
        all_imports.add(
            f"import 'package:{get_project_package_name()}/{CUSTOM_DIRNAME}/routes.dart' as {CUSTOM_ROUTES_IMPORT_ALIAS};"
        )
        all_routes.append(f"    ...{CUSTOM_ROUTES_IMPORT_ALIAS}.customRoutes,")
...
def update_brand_hook():
    ...
    # The app's own theme override (custom/theme.dart) runs after the SDK
    # brand hook so it wins over the SDK palette.
    custom_theme = has_custom_file("theme.dart")
    if custom_theme:
        body_lines.append(f"  {CUSTOM_THEME_IMPORT_ALIAS}.applyCustomTheme();")
    ...
    if "// @generated-brandhook-start" not in content:
        if custom_theme:
            compose_warning(
                ...
                f"(older host template); custom/theme.dart NOT applied"
            )
        if declared:
            compose_warning(
                f"marker // @generated-brandhook-start not found in {MAIN_FILE} "
    ...
    for pkg_name, pkg_data in state.get("packages", {}).items():
        ...
        for entry in wiring_entries:
            for imp in entry.get("imports", []) or []:
                ...
                if imp:
                    all_imports.add(imp)
    if has_custom_file("theme.dart"):
        all_imports.add(
            f"import 'package:{package_name}/{CUSTOM_DIRNAME}/theme.dart' as {CUSTOM_THEME_IMPORT_ALIAS};"
        )
