...
class Question:
    ...
    def __init__(self, label, prompt, default=None, required=False, example=None,
                 tier=TIER_CORE):
        self.label = label
        self.prompt = prompt
...
BUSINESS_SCHEMA = [
    Section("Venture Identity & Jurisdiction", [
        Question("Trading Name", "What is the primary commercial brand or trading name?",
                 required=True),
        Question(
            "Jurisdiction",
        ...
        Question("Primary Base", "What is the primary geographical base of operations?",
                 required=True),
        Question("Establishment Date", "When was the venture established?"),
        Question("Industry", "Which industry or sector does the venture operate in?",
                 required=True, example="Healthcare services"),
        Question("Vision Statement", "What is the venture's core vision?", required=True),
        Question("Core Value Proposition",
                 "What is the product or service's unique value statement?", required=True),
    ]),
    Section("Market & Product", [
        Question("Primary Products", "What are the core components of the product or service suite?",
                 required=True),
        Question("Customer Segments", "Who are the primary target customers?", required=True),
        Question("Growth Strategy", "What is the key customer acquisition loop?", required=True),
    ...
    Section("Operations & People", [
        Question("Key Suppliers", "Who are the primary strategic suppliers or vendors?"),
        Question("Personnel Count", "What is the current headcount?"),
        Question("Board Directors", "Who are the active directors on the board?"),
        Question("Shareholder Distribution", "What is the shareholder percentage split?"),
        Question("Key Operational Risks", "What are the top operational risks and their mitigations?"),
        Question("Business Continuity Strategy",
                 "How does the venture handle its main infrastructure or supply disruption risk?"),
    ]),
    Section("Financial Projections & History", [
        ...
        Question("Funding Requirement", "How much capital is being sought, and for what?"),
    ]),

    # ---- Extended set: needed by the full business-plan suite ----

    Section("Mission & Philosophy", [
        Question("Mission Statement", "What does the venture do, for whom, and why?",
                 tier=TIER_FULL),
        Question("Core Philosophy",
                 "What principles govern how the venture builds and operates? "
                 "One line per principle.", tier=TIER_FULL),
        Question("Head Office", "Where is the registered head office or main premises?",
                 tier=TIER_FULL),
        Question("Secondary Locations", "Any additional sites, branches or field offices?",
                 tier=TIER_FULL),
        Question("Target Sectors", "Which sectors or markets does the venture sell into?",
                 tier=TIER_FULL),
        Question("Intellectual Property",
                 "What proprietary assets exist — patents, trademarks, software, "
                 "recipes, methods, licences?", tier=TIER_FULL),
    ]),

    Section("Market & Opportunity", [
        Question("Problem Statement",
                 "What specific problem does the venture solve? Name who has it "
                 "and what it costs them today.", tier=TIER_FULL),
        Question("Market Size TAM",
                 "Total addressable market — the whole category, with a source.",
                 tier=TIER_FULL),
        Question("Market Size SAM",
                 "Serviceable addressable market — the slice reachable with the "
                 "current model.", tier=TIER_FULL),
        Question("Market Size SOM",
                 "Serviceable obtainable market — realistic capture over 36 months.",
                 tier=TIER_FULL),
        Question("Market Trends", "Which trends make this the right moment?",
                 tier=TIER_FULL),
        Question("Competitive Positioning",
                 "How does the venture compare to each named competitor? "
                 "One line per competitor.", tier=TIER_FULL),
        Question("Customer Segment Primary",
                 "Describe the primary customer: who they are, what they buy, how "
                 "often.", tier=TIER_FULL),
        Question("Customer Segment Secondary", "The second customer group, if any.",
                 tier=TIER_FULL),
        Question("Customer Segment Tertiary", "A third customer group, if any.",
                 tier=TIER_FULL),
    ]),

    Section("Product & Technology", [
        Question("Product Components",
        ...
        Question("Technical Architecture",
                 ...
                 tier=TIER_FULL),
        Question("Product Roadmap", "What ships in the next 12-24 months?",
                 tier=TIER_FULL),
        Question("Hardware Or Equipment",
                 "Any physical hardware, machinery or devices involved?",
                 tier=TIER_FULL),
    ]),

    Section("Marketing & Sales", [
        Question("Acquisition Channels",
                 "Through which channels do customers actually arrive? One per line.",
                 tier=TIER_FULL),
        Question("Pricing Tiers",
                 "What does the venture charge, per tier or product line?",
                 tier=TIER_FULL),
        Question("Sales Process", "From first contact to signed customer, what happens?",
                 tier=TIER_FULL),
        Question("Brand Positioning", "In one sentence, how should the market see you?",
                 tier=TIER_FULL),
        Question("Marketing Budget", "What is committed to marketing, and over what period?",
                 tier=TIER_FULL),
    ]),

    Section("Operations & Quality", [
        Question("Key Processes",
                 "What are the core repeatable operations, from order to delivery?",
                 tier=TIER_FULL),
        Question("Quality Standards",
                 "Which standards, certifications or inspections apply?",
                 tier=TIER_FULL),
        Question("Service Levels", "What does the venture commit to on delivery or uptime?",
                 tier=TIER_FULL),
        Question("Capacity Constraints", "What limits how much the venture can deliver today?",
                 tier=TIER_FULL),
    ]),

    Section("People & Governance", [
        Question("Executive Team",
                 "Who holds each executive role, and what do they own? "
                 "One line per person.", tier=TIER_FULL),
        Question("HR Vision", "What kind of team is being built, and how?", tier=TIER_FULL),
        Question("Hiring Plan", "Which roles are being hired, when, and at what cost?",
                 tier=TIER_FULL),
        Question("Organisational Culture", "What behaviour is expected and rewarded?",
                 tier=TIER_FULL),
        Question("Key Person Dependencies",
                 "Which individuals would the venture struggle to replace?",
                 tier=TIER_FULL),
        Question("Succession Arrangements",
    ...
    Section("Financial Detail", [
        Question("Revenue Streams",
                 ...
                 tier=TIER_FULL),
        Question("Cost Structure", "The main fixed and variable cost lines.", tier=TIER_FULL),
        Question("Gross Margin Target", "Target gross margin, by line if they differ.",
                 tier=TIER_FULL),
        Question("Break Even Point", "What revenue or volume covers fixed costs?",
                 tier=TIER_FULL),
        Question("Capital Allocation", "How would new capital be spent, by category?",
                 tier=TIER_FULL),
        Question("Funding History", "What has been raised or granted so far, from whom?",
                 tier=TIER_FULL),
    ]),

    Section("Strategy & Milestones", [
        Question("Strategic Objectives", "The three to five objectives for the year.",
                 tier=TIER_FULL),
        Question("Milestones 12 Month", "What must be true 12 months from now?",
                 tier=TIER_FULL),
        Question("Milestones 36 Month", "What must be true in three years?",
                 tier=TIER_FULL),
        Question("Key Projects", "Named projects in flight, with owner and due date.",
                 tier=TIER_FULL),
        Question("Achievements To Date", "Wins worth citing: grants, pilots, awards, contracts.",
                 tier=TIER_FULL),
    ]),

    Section("Commercial Terms", [
        Question("Payment Terms", "Deposit, credit period, late-payment terms?",
                 tier=TIER_FULL),
        Question("Delivery Terms", "Lead times, delivery responsibility, risk transfer?",
                 tier=TIER_FULL),
        Question("Warranty Terms", "What is warranted, for how long, with what remedy?",
                 tier=TIER_FULL),
        Question("Returns Policy", "Under what conditions are returns or refunds accepted?",
                 tier=TIER_FULL),
        Question("Dispute Resolution", "How are disputes handled, and under which law?",
                 tier=TIER_FULL),
    ]),
]
...
LIFE_SCHEMA = [
    Section("Personal Identity & Focus", [
        Question("Full Name", "What is your full name?", required=True),
        Question("Pronouns", "Which pronouns should documents use? (e.g. she/her, he/him, they/them)",
                 required=True, example="they/them"),
        Question("Primary Base", "What is your primary geographical base?", required=True),
        Question("Jurisdiction", "Which country are you resident in? Use an ISO code.",
                 example="ZA"),
        Question("Life Purpose", "What is your core mission or purpose statement?", required=True),
        Question("Wellness Focus", "What is your primary wellness or performance goal?"),
    ]),
    Section("Relationships & Stewardship", [
        Question("Key Relationships", "Who are the primary partners, confidants or trustees?"),
    ...
    Section("Venture & Career Integration", [
        Question("Business Ownership", "Do you own a registered business or run a side venture?"),
        Question("Current Role", "What is your current professional role?"),
        Question("Skill Focus", "Which capability are you deliberately building right now?"),
    ]),
    Section("Productivity & Rhythm", [
...
def validate_schema_against_templates(instance_type, template_placeholders,
                                      engine_supplied=()):
    ...
    uncollected = sorted(
        name for name in template_placeholders
