    ...
    if job_dir.exists():
        for f in job_dir.glob("*.md"):
            ...
            with open(f, "r") as content:
                ...
                if theme_match:
                    themes.add(theme_match.group(1).strip())
...
def run_lock(args):
    ...
    if args.action == "check":
        ...
        if not session_id or is_stale:
            ...
            content = set_field(
                content, "session_started", now.strftime("%Y-%m-%d %H:%M:%S")
            )
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(content)
    ...
    if args.status:
        ...
        if current_status and current_status in ALLOWED_TRANSITIONS:
            if args.status not in ALLOWED_TRANSITIONS[current_status]:
                print(
                    f"Error: Invalid transition from {current_status} to {args.status}"
                )
                sys.exit(1)
        elif current_status:
            if args.status not in ["failed", "stalled"]:
                print(
                    f"Error: Unknown current status {current_status}. Transition to {args.status} rejected."
                )
                sys.exit(1)
        ...
        log_entries.append(
            f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] {card_id} | {current_status} -> {args.status} | {args.agent}"
        )
    ...
    if args.visuals_status:
        ...
        if current_val and current_val in VISUALS_TRANSITIONS:
            if args.visuals_status not in VISUALS_TRANSITIONS[current_val]:
                print(
                    f"Error: Invalid transition from {current_val} to {args.visuals_status}"
                )
                sys.exit(1)

        content = set_field(content, field, args.visuals_status)
        log_entries.append(
            f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] {card_id} | {field}: {current_val} -> {args.visuals_status} | {args.agent}"
        )
...
def main():
    ...
    lock_parser = subparsers.add_parser(
        "lock", help="Lock/claim or release job card lock"
    )
    ...
    status_parser = subparsers.add_parser(
        "status", help="Update card status with validation"
    )
